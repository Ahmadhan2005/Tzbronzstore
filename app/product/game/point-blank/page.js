...
// Data produk Point Blank Cash
const pbProductOptions = [
  { label: "1.200 Cash", price: "Rp. 12.000" },
  { label: "2.400 Cash", price: "Rp. 24.000" },
  { label: "6.000 Cash", price: "Rp. 54.000" },
  { label: "12.000 Cash", price: "Rp. 100.000" },
  { label: "24.000 Cash", price: "Rp. 194.000" },
  { label: "36.000 Cash", price: "Rp. 230.000" },
  { label: "60.000 Cash", price: "Rp. 500.000" },
];
...
const pbGameInfo = {
  ...
  desc: "Top up Point Blank Cash untuk membeli item, senjata, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(pbProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Point Blank:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={pbGameInfo.image}
              alt={pbGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{pbGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{pbGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Point Blank</label>
            <input
              type="text"
              value={userId}
              onChange={(e) => setUserId(e.target.value)}
              className={inputClass}
              placeholder="Contoh: Username Point Blank"
            />
          </div>

          {/* Step 2: Pilih Paket Cash */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Cash</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {pbProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
