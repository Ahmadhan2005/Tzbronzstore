...
// Data produk B-Chips Honkai Impact 3
const hi3BChipsOptions = [
  { label: "65 B-Chips", price: "Rp. 18.500" },
  { label: "330 B-Chips", price: "Rp. 81.000" },
  { label: "990 B-Chips", price: "Rp. 253.000" },
  { label: "1320 B-Chips", price: "Rp. 334.000" },
  { label: "1980 B-Chips", price: "Rp. 483.000" },
  { label: "3300 B-Chips", price: "Rp. 810.000" },
  { label: "6600 B-Chips", price: "Rp. 1.600.000" },
];

// Data produk Crystals Honkai Impact 3
const hi3CrystalsOptions = [
  { label: "65 Crystals", price: "Rp. 18.500" },
  { label: "330 Crystals", price: "Rp. 81.000" },
  { label: "710 Crystals", price: "Rp. 165.000" },
  { label: "1430 Crystals", price: "Rp. 340.000" },
  { label: "3860 Crystals", price: "Rp. 810.000" },
  { label: "8088 Crystals", price: "Rp. 1.600.000" },
];

// Data info game
const hi3GameInfo = {
  name: "Honkai Impact 3",
  image: "/HONKAI IMPACT.png",
  desc: "Top up B-Chips dan Crystals Honkai Impact 3 untuk membeli karakter, equipment, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [type, setType] = useState("bchips"); // bchips | crystals
  const [product, setProduct] = useState(hi3BChipsOptions[0].label);
  ...
  const handleTypeChange = (newType) => {
    ...
    setProduct(
      newType === "bchips"
        ? hi3BChipsOptions[0].label
        : hi3CrystalsOptions[0].label
    );
  };
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Honkai Impact 3:\n` +
        `• User ID: ${userId}\n` +
        `• Tipe: ${type === "bchips" ? "B-Chips" : "Crystals"}\n` +
        `• Paket: ${product}\n` +
  ...
  const productOptions = type === "bchips" ? hi3BChipsOptions : hi3CrystalsOptions;
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={hi3GameInfo.image}
              alt={hi3GameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{hi3GameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{hi3GameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Honkai Impact 3</label>
            <input
              type="text"
              value={userId}
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-4 mb-2">
              <button
                type="button"
                onClick={() => handleTypeChange("bchips")}
              ...
              <button
                type="button"
                onClick={() => handleTypeChange("crystals")}
                className={`px-4 py-2 rounded-lg font-bold text-sm border-2 transition-all duration-200 ${
                  type === "crystals"
                    ? "border-purple-600 bg-purple-100 text-purple-700 scale-105 shadow"
                    : "border-purple-200 bg-purple-50 text-purple-500 hover:border-purple-400 hover:bg-purple-100 hover:text-purple-700"
