...
// Data produk Blood Strike - Gold
const bloodStrikeGoldOptions = [
  { label: "100 Gold", price: "Rp. 13.000" },
  { label: "300 Gold", price: "Rp. 36.000" },
  { label: "500 Gold", price: "Rp. 58.500" },
  { label: "1000 Gold", price: "Rp. 114.000" },
  { label: "2000 Gold", price: "Rp. 227.000" },
  { label: "5000 Gold", price: "Rp. 540.500" },
];

// Data produk Blood Strike - Pass
const bloodStrikePassOptions = [
  { label: "Strike pass elite", price: "Rp. 47.000" },
  { label: "Strike pass premium", price: "Rp. 105.000" },
  { label: "Level up pass", price: "Rp. 25.500" },
];
...
export default function ProductPage() {
  ...
  const [productType, setProductType] = useState("gold"); // "gold" | "pass"
  const [product, setProduct] = useState(bloodStrikeGoldOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      ...
      if (productType === "gold") {
        pesan =
          `Halo Admin, saya ingin top up Gold Blood Strike:\n` +
          `• User ID: ${userId}\n` +
          `• Paket: ${product}\n` +
        ...
        pesan =
          `Halo Admin, saya ingin order Pass Blood Strike:\n` +
          `• User ID: ${userId}\n` +
          `• Pass: ${product}\n` +
          `• Pembayaran: ${paymentOptions.find((p) => p.value === payment)?.label}\n` +
          `Terima kasih.`;
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={bloodStrikeGameInfo.image}
              alt={bloodStrikeGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{bloodStrikeGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{bloodStrikeGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6 flex gap-4 flex-wrap">
            <button
              ...
              onClick={() => {
                setProductType("gold");
                setProduct(bloodStrikeGoldOptions[0].label);
              }}
            ...
            <button
              ...
              onClick={() => {
                setProductType("pass");
                setProduct(bloodStrikePassOptions[0].label);
              }}
              className={`px-4 py-2 rounded-lg font-bold text-sm border-2 transition-all duration-200 ${
                productType === "pass"
                  ? "border-purple-600 bg-purple-100 text-purple-700 scale-105 shadow"
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Blood Strike</label>
            <input
              type="text"
              value={userId}
          ...
          <div className="mb-6">
            <label className={labelClass}>
              {productType === "gold" ? "Pilih Paket Gold" : "Pilih Paket Pass"}
            </label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {(productType === "gold" ? bloodStrikeGoldOptions : bloodStrikePassOptions).map((opt) => (
                <button
                  key={opt.label}
                  type="button"
