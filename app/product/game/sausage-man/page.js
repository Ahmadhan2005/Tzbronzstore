...
// Data produk Sausage Man Candy
const sausageManProductOptions = [
  { label: "61 Candy", price: "Rp. 14.500" },
  { label: "186 Candy", price: "Rp. 40.500" },
  { label: "318 Candy", price: "Rp. 68.500" },
  { label: "686 Candy", price: "Rp. 134.500" },
  { label: "1378 Candy", price: "Rp. 268.000" },
  { label: "2118 Candy", price: "Rp. 396.000" },
  { label: "3548 Candy", price: "Rp. 663.000" },
  { label: "7108 Candy", price: "Rp. 1.300.000" },
];

// Data info game
const sausageManGameInfo = {
  name: "Sausage Man",
  image: "/SAUSAGE MAN.png",
  desc: "Top up Candy Sausage Man untuk membeli item, skin, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(sausageManProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Sausage Man:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={sausageManGameInfo.image}
              alt={sausageManGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{sausageManGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{sausageManGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Sausage Man</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Candy */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Candy</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {sausageManProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
