...
// Data produk Gems Nikke: Goddess of Victory
const nikkeProductOptions = [
  { label: "6 + 6 Gems", price: "Rp. 3.000" },
  { label: "60 + 1 Gems", price: "Rp. 15.000" },
  { label: "120 + 3 Gems", price: "Rp. 28.500" },
  { label: "320 + 10 Gems", price: "Rp. 66.000" },
  { label: "720 + 120 Gems", price: "Rp. 130.000" },
  { label: "1500 + 260 Gems", price: "Rp. 259.000" },
  { label: "2300 + 400 Gems", price: "Rp. 385.000" },
  { label: "2370 + 420 Gems", price: "Rp. 405.000" },
  { label: "4200 + 1000 Gems", price: "Rp. 720.000" },
  { label: "6200 + 1500 Gems", price: "Rp. 1.150.000" },
];
...
const nikkeGameInfo = {
  ...
  image: "/NIKKE.png",
  desc: "Top up Gems Nikke: Goddess of Victory untuk membeli karakter, skin, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(nikkeProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Nikke: Goddess of Victory:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={nikkeGameInfo.image}
              alt={nikkeGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{nikkeGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{nikkeGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Nikke: Goddess of Victory</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Gems */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Gems</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {nikkeProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
