...
// Data produk Zenless Zone Zero (Monochrome)
const zzzProductOptions = [
  { label: "60 Monochrome", price: "Rp. 15.000" },
  { label: "300 + 30 Monochrome", price: "Rp. 67.000" },
  { label: "980 + 110 Monochrome", price: "Rp. 194.500" },
  { label: "1980 + 260 Monochrome", price: "Rp. 384.000" },
  { label: "3280 + 600 Monochrome", price: "Rp. 626.000" },
  { label: "6480 + 1600 Monochrome", price: "Rp. 1.300.000" },
];
...
const zzzGameInfo = {
  ...
  image: "/ZZZ.png",
  desc: "Top up Monochrome Zenless Zone Zero (ZZZ) untuk membeli item, karakter, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(zzzProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Zenless Zone Zero (ZZZ):\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={zzzGameInfo.image}
              alt={zzzGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{zzzGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{zzzGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Zenless Zone Zero</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Monochrome */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Monochrome</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {zzzProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
