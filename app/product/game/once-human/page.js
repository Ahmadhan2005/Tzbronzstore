...
// Data produk Once Human Crystgin
const onceHumanProductOptions = [
  { label: "62 Crystgin", price: "Rp. 18.000" },
  { label: "339 Crystgin", price: "Rp. 84.000" },
  { label: "1120 Crystgin", price: "Rp. 260.000" },
  { label: "2340 Crystgin", price: "Rp. 500.000" },
  { label: "3979 Crystgin", price: "Rp. 840.000" },
  { label: "8075 Crystgin", price: "Rp. 1.700.000" },
];

// Data info game
const onceHumanGameInfo = {
  name: "Once Human",
  image: "/ONCE HUMAN.png",
  desc: "Top up Crystgin Once Human untuk membeli item, skin, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(onceHumanProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Once Human:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={onceHumanGameInfo.image}
              alt={onceHumanGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{onceHumanGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{onceHumanGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Once Human</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Crystgin */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Crystgin</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {onceHumanProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
