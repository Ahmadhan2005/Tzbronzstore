...
// Data produk League of Legends Wild Rift Cores
const wildRiftProductOptions = [
  { label: "425 Cores", price: "Rp. 55.000" },
  { label: "1000 Cores", price: "Rp. 123.000" },
  { label: "1850 Cores", price: "Rp. 220.000" },
  { label: "3275 Cores", price: "Rp. 382.000" },
  { label: "4800 Cores", price: "Rp. 550.000" },
  { label: "10000 Cores", price: "Rp. 1.100.000" },
];

// Data info game
const wildRiftGameInfo = {
  name: "League of Legends: Wild Rift",
  image: "/WILD RIFT.png",
  desc: "Top up Wild Cores League of Legends: Wild Rift untuk membeli champion, skin, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(wildRiftProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order League of Legends: Wild Rift:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={wildRiftGameInfo.image}
              alt={wildRiftGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{wildRiftGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{wildRiftGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Wild Rift</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Wild Cores */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Wild Cores</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {wildRiftProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
