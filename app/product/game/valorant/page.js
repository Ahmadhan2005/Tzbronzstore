...
// Data produk Valorant Points
const valorantProductOptions = [
  { label: "475 VP", price: "Rp. 58.500" },
  { label: "1000 VP", price: "Rp. 112.000" },
  { label: "1475 VP", price: "Rp. 169.000" },
  { label: "2050 VP", price: "Rp. 220.500" },
  { label: "2525 VP", price: "Rp. 283.500" },
  { label: "3050 VP", price: "Rp. 334.000" },
  { label: "3650 VP", price: "Rp. 382.500" },
  { label: "4125 VP", price: "Rp. 441.500" },
  { label: "4650 VP", price: "Rp. 499.000" },
  { label: "5350 VP", price: "Rp. 550.000" },
];

// Data info game
const valorantGameInfo = {
  name: "Valorant",
  image: "/VALO.png",
  desc: "Top up Valorant Points untuk membeli skin, battle pass, dan item in-game lainnya. Harga terbaik & proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(valorantProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Valorant Points:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={valorantGameInfo.image}
              alt={valorantGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{valorantGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{valorantGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Valorant</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket VP */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Valorant Points (VP)</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {valorantProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
