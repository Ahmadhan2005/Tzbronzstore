...
// Data produk Delta Force Garena Points
const deltaForceProductOptions = [
  { label: "18 + 1 Points", price: "Rp. 6.500" },
  { label: "30 + 2 Points", price: "Rp. 9.500" },
  { label: "60 + 3 Points", price: "Rp. 17.500" },
  { label: "300 + 36 Points", price: "Rp. 79.000" },
  { label: "420 + 62 Points", price: "Rp. 109.500" },
  { label: "680 + 105 Points", price: "Rp. 155.000" },
  { label: "1280 + 385 Points", price: "Rp. 294.000" },
  { label: "3280 + 834 Points", price: "Rp. 718.500" },
  { label: "6480 + 1944 Points", price: "Rp. 1.500.000" },
  { label: "12960 + 3888 Points", price: "Rp. 3.000.000" },
];

// Data info game
const deltaForceGameInfo = {
  name: "Delta Force Garena",
  image: "/Delta Force.png",
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(deltaForceProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Delta Force Garena:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={deltaForceGameInfo.image}
              alt={deltaForceGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{deltaForceGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{deltaForceGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Delta Force Garena</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Points */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Points</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {deltaForceProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
