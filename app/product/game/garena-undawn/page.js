...
// Data produk RC Garena Undawn
const undawnProductOptions = [
  { label: "80 RC", price: "Rp. 14.500" },
  { label: "250 RC", price: "Rp. 40.500" },
  { label: "450 RC", price: "Rp. 68.000" },
  { label: "920 RC", price: "Rp. 132.000" },
  { label: "1850 RC", price: "Rp. 265.000" },
  { label: "2800 RC", price: "Rp. 395.000" },
  { label: "4750 RC", price: "Rp. 660.000" },
  { label: "9600 RC", price: "Rp. 1.300.000" },
];

// Data info game
const undawnGameInfo = {
  name: "Garena Undawn",
  image: "/UNDAWN.png",
  desc: "Top up RC Garena Undawn untuk membeli item, skin, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(undawnProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Garena Undawn:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={undawnGameInfo.image}
              alt={undawnGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{undawnGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{undawnGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Garena Undawn</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket RC */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket RC</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {undawnProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
