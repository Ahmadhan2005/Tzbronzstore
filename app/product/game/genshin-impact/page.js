...
// Data produk Genesis Crystals Genshin Impact
const genshinProductOptions = [
  { label: "60 Genesis Crystals", price: "Rp. 13.500" },
  { label: "300 + 30 Genesis Crystals", price: "Rp. 66.000" },
  { label: "980 + 110 Genesis Crystals", price: "Rp. 195.000" },
  { label: "1280 + 140 Genesis Crystals", price: "Rp. 259.500" },
  { label: "1980 + 260 Genesis Crystals", price: "Rp. 415.500" },
  { label: "3280 + 600 Genesis Crystals", price: "Rp. 631.000" },
  { label: "4260 + 710 Genesis Crystals", price: "Rp. 810.000" },
  { label: "6480 + 1600 Genesis Crystals", price: "Rp. 1.230.000" },
  { label: "9760 + 2200 Genesis Crystals", price: "Rp. 1.850.000" },
  { label: "10740 + 2310 Genesis Crystals", price: "Rp. 2.050.000" },
];

// Data info game
const genshinGameInfo = {
  name: "Genshin Impact",
  image: "/Genshin Impact.png",
  desc: "Top up Genesis Crystals Genshin Impact untuk membeli karakter, senjata, dan item in-game lainnya. Harga terbaik & proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(genshinProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Genshin Impact:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={genshinGameInfo.image}
              alt={genshinGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{genshinGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{genshinGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Genshin Impact</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Genesis Crystals */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Genesis Crystals</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {genshinProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
