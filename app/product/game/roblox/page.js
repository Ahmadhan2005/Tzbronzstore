...
// Data produk Top Up Robux Roblox
const robloxProductOptions = [
  { label: "800 Robux", price: "Rp. 179.000" },
  { label: "2000 Robux", price: "Rp. 431.000" },
  { label: "4500 Robux", price: "Rp. 810.000" },
  { label: "10000 Robux", price: "Rp. 1.750.000" },
];

// Data info game
const robloxGameInfo = {
  name: "Roblox",
  image: "/ROBLOX.png",
  desc: "Top up Robux Roblox untuk membeli item, avatar, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(robloxProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Roblox:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={robloxGameInfo.image}
              alt={robloxGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{robloxGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{robloxGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Roblox</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Robux */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Robux</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {robloxProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
