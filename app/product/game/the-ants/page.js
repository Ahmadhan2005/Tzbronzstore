...
// Data produk The Ants Coin
const theAntsProductOptions = [
  { label: "22 Coin", price: "Rp. 6.000" },
  { label: "100 Coin", price: "Rp. 18.500" },
  { label: "500 Coin", price: "Rp. 78.500" },
  { label: "1000 Coin", price: "Rp. 155.000" },
  { label: "1500 Coin", price: "Rp. 240.000" },
  { label: "2000 Coin", price: "Rp. 320.000" },
  { label: "2500 Coin", price: "Rp. 386.000" },
  { label: "3000 Coin", price: "Rp. 462.000" },
];

// Data info game
const theAntsGameInfo = {
  name: "The Ants",
  image: "/THE ANTS.png",
  desc: "Top up Coin The Ants untuk membeli item, upgrade, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(theAntsProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order The Ants:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={theAntsGameInfo.image}
              alt={theAntsGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{theAntsGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{theAntsGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID The Ants</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Coin */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Coin</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {theAntsProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
