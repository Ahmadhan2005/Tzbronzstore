...
// Data produk Punishing: Gray Raven - Rainbow Cards (Android & iOS)
const pgrProductOptions = [
  { label: "5 Rainbow Cards", price: "Rp. 14.500" },
  { label: "28 Rainbow Cards", price: "Rp. 67.500" },
  { label: "34 Rainbow Cards", price: "Rp. 86.000" },
  { label: "59 Rainbow Cards", price: "Rp. 139.000" },
  { label: "71 Rainbow Cards", price: "Rp. 164.500" },
  { label: "119 Rainbow Cards", price: "Rp. 275.500" },
  { label: "299 Rainbow Cards", price: "Rp. 685.000" },
  { label: "600 Rainbow Cards", price: "Rp. 1.400.000" },
];
...
const pgrGameInfo = {
  ...
  image: "/PGR.png",
  desc: "Top up Rainbow Cards Punishing: Gray Raven (PGR) untuk Android & iOS. Harga terbaik & proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [device, setDevice] = useState("android"); // android | ios
  const [product, setProduct] = useState(pgrProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Punishing: Gray Raven (PGR):\n` +
        `• User ID: ${userId}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={pgrGameInfo.image}
              alt={pgrGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{pgrGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{pgrGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Punishing: Gray Raven</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Rainbow Cards */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Rainbow Cards</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {pgrProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
