...
// Data produk Honkai Star Rail - Oneiric Shard
const hsrProductOptions = [
  { label: "60 Oneiric Shard", price: "Rp. 13.500" },
  { label: "300 + 30 Oneiric Shard", price: "Rp. 59.000" },
  { label: "980 + 110 Oneiric Shard", price: "Rp. 193.500" },
  { label: "1980 + 260 Oneiric Shard", price: "Rp. 379.500" },
  { label: "3280 + 600 Oneiric Shard", price: "Rp. 620.000" },
  { label: "6480 + 1600 Oneiric Shard", price: "Rp. 1.200.000" },
];

// Data info game
const hsrGameInfo = {
  name: "Honkai Star Rail",
  image: "/honkai starrail.png",
  desc: "Top up Oneiric Shard Honkai Star Rail untuk membeli item, karakter, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [product, setProduct] = useState(hsrProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Honkai Star Rail:\n` +
        `• User ID: ${userId}\n` +
        `• Paket: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={hsrGameInfo.image}
              alt={hsrGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{hsrGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{hsrGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Honkai Star Rail</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Oneiric Shard */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Oneiric Shard</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {hsrProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
