...
// Data produk Goldstar Super SUS
const superSusCrystalsOptions = [
  { label: "100 Crystals", price: "Rp. 12.500" },
  { label: "310 Crystals", price: "Rp. 34.000" },
  { label: "520 Crystals", price: "Rp. 58.000" },
  { label: "1060 Crystals", price: "Rp. 112.000" },
  { label: "2180 Crystals", price: "Rp. 226.000" },
  { label: "5600 Crystals", price: "Rp. 590.000" },
];

// Data produk Item Super SUS
const superSusItemOptions = [
  { label: "Super Sus Weekly Card", price: "Rp. 13.000" },
  { label: "Super Sus Monthly Card", price: "Rp. 145.000" },
  { label: "Super Sus Super Pass", price: "Rp. 60.000" },
  { label: "Super Sus Super VIP Card", price: "Rp. 160.000" },
  { label: "Super Sus Super Pass Bundle", price: "Rp. 115.000" },
];

// Data info game
const superSusGameInfo = {
  name: "Super SUS",
  image: "/SUPERSUS.png",
  desc: "Top up Goldstar Crystals dan Item Super SUS untuk membeli item, pass, dan keperluan in-game lainnya dengan harga terbaik dan proses instan di tzbronzstore.id.",
};
...
export default function ProductPage() {
  ...
  const [type, setType] = useState("crystals"); // crystals | item
  const [product, setProduct] = useState(superSusCrystalsOptions[0].label);
  ...
  const handleTypeChange = (newType) => {
    ...
    setProduct(
      newType === "crystals"
        ? superSusCrystalsOptions[0].label
        : superSusItemOptions[0].label
    );
  };
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Super SUS:\n` +
        `• User ID: ${userId}\n` +
        `• Tipe: ${type === "crystals" ? "Goldstar Crystals" : "Item"}\n` +
        `• Paket: ${product}\n` +
  ...
  const productOptions = type === "crystals" ? superSusCrystalsOptions : superSusItemOptions;
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={superSusGameInfo.image}
              alt={superSusGameInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{superSusGameInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{superSusGameInfo.desc}</p>
        </div>
        ...
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          ...
          <div className="mb-6">
            <label className={labelClass}>Masukkan User ID Super SUS</label>
            <input
              type="text"
              value={userId}
