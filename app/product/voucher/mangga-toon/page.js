...
// Data produk voucher Mangga Toon
const manggaToonOptions = [
  { label: "100 Coins", price: "Rp. 17.000" },
  { label: "600 Coins", price: "Rp. 95.000" },
  { label: "1000 Coins", price: "Rp. 160.000" },
  { label: "6000 Coins", price: "Rp. 940.000" },
  { label: "10000 Coins", price: "Rp. 1.650.000" },
];

// Data info voucher
const manggaToonInfo = {
  name: "Mangga Toon",
  image: "/voucher/Mangga toon.jpg",
  desc: "Voucher Mangga Toon untuk membeli koin dan menikmati komik favoritmu tanpa batas. Pilih jumlah koin sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Username/email Mangga Toon
  const [product, setProduct] = useState(manggaToonOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Mangga Toon:\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={manggaToonInfo.image}
              alt={manggaToonInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{manggaToonInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{manggaToonInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Username/Email Mangga Toon */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Username/Email Mangga Toon</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Coins */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Coins</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {manggaToonOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
