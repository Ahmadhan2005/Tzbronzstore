...
// Data produk Steam Wallet
const steamWalletOptions = [
  { label: "45.000", price: "Rp. 47.000" },
  { label: "60.000", price: "Rp. 63.000" },
  { label: "90.000", price: "Rp. 92.000" },
  { label: "120.000", price: "Rp. 122.000" },
  { label: "250.000", price: "Rp. 252.000" },
  { label: "400.000", price: "Rp. 402.000" },
  { label: "600.000", price: "Rp. 602.000" },
];
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Email/akun Steam
  const [product, setProduct] = useState(steamWalletOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Steam Wallet:\n` +
        `• Email/Akun: ${userId}\n` +
        `• Nominal: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={steamWalletInfo.image}
              alt={steamWalletInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{steamWalletInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{steamWalletInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Email/Akun Steam */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Email/Akun Steam</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Nominal Wallet */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Nominal Wallet</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {steamWalletOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
