...
// Data produk voucher Vidio
const vidioProductOptions = [
  { label: "Platinum 30 Hari", price: "Rp. 40.000" },
  { label: "Diamond Mobile 30 Hari", price: "Rp. 53.000" },
  { label: "Diamond All Screen 30 Hari", price: "Rp. 145.000" },
  { label: "Diamond Mobile 1 Tahun", price: "Rp. 555.000" },
];

// Data info voucher
const vidioInfo = {
  name: "Vidio",
  image: "/voucher/vidio.png",
  desc: "Voucher Vidio untuk akses streaming premium berbagai channel, olahraga, dan hiburan. Pilih paket sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Email/akun Vidio
  const [product, setProduct] = useState(vidioProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Vidio:\n` +
        `• Email/Akun: ${userId}\n` +
        `• Paket: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{vidioInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{vidioInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Email/Akun Vidio */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Email/Akun Vidio</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket Voucher */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Voucher</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {vidioProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
