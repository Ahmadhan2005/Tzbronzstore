...
// Data produk voucher Bstation
const voucherOptions = [
  { label: "7 days", price: "Rp9.500" },
  { label: "14 days", price: "Rp14.000" },
  { label: "31 days", price: "Rp24.000" },
  { label: "93 days", price: "Rp68.000" },
  { label: "366 days", price: "Rp200.000" },
];

// Data info voucher
const voucherInfo = {
  name: "Bstation Premium",
  image: "/voucher/bstation.png",
  desc: "Bstation Premium Voucher untuk akses eksklusif anime, drama, dan konten favoritmu tanpa batas. Pilih masa aktif sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // userId bisa diganti dengan username/email Bstation
  const [product, setProduct] = useState(voucherOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Bstation Premium:\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{voucherInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{voucherInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Username/Email Bstation */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Username/Email Bstation</label>
            <input
              type="text"
              value={userId}
          ...
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Premium</label>
