...
// Data produk voucher Xbox Membership
const xboxProductOptions = [
  { label: "3 Months", price: "Rp. 370.000" },
];

// Data info Xbox
const xboxInfo = {
  name: "Xbox Membership",
  image: "/voucher/xbox.png",
  desc: "Top Up Xbox Membership untuk akses game dan fitur premium di Xbox. Pilih masa aktif sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Email/akun Xbox
  const [product, setProduct] = useState(xboxProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Xbox Membership:\n` +
        `• Email/Akun: ${userId}\n` +
        `• Paket: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{xboxInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{xboxInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Email/Akun Xbox */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Email/Akun Xbox</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Membership */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Membership</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {xboxProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
