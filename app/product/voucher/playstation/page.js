...
// Data produk voucher PlayStation Network (PSN)
const psnOptions = [
  { label: "100.000", price: "Rp. 102.000" },
  { label: "225.000", price: "Rp. 270.000" },
];

// Data info voucher
const psnInfo = {
  name: "PlayStation Network (PSN)",
  image: "/voucher/playstation .jpg",
  desc: "Voucher PlayStation Network untuk pembelian game, DLC, dan konten digital di PlayStation Store. Pilih nominal voucher sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Email/akun PSN
  const [product, setProduct] = useState(psnOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order PlayStation Network (PSN):\n` +
        `• Email/Akun: ${userId}\n` +
        `• Nominal: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{psnInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{psnInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Email/Akun PSN */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Email/Akun PlayStation</label>
            <input
              type="text"
              value={userId}
