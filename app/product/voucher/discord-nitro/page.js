...
// Data produk voucher Discord Nitro
const nitroOptions = [
  { label: "Discord Nitro Basic Monthly", price: "Rp. 95.000" },
  { label: "Discord Nitro Monthly", price: "Rp. 190.000" },
  { label: "Discord Nitro Yearly", price: "Rp. 1.900.000" },
];

// Data info voucher
const nitroInfo = {
  name: "Discord Nitro",
  image: "/voucher/discord nitro.jpg",
  desc: "Dapatkan Discord Nitro untuk akses fitur premium seperti upload file besar, badge eksklusif, dan banyak lagi. Pilih paket sesuai kebutuhanmu, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Discord username/email
  const [product, setProduct] = useState(nitroOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Discord Nitro:\n` +
        `• Username/Email Discord: ${userId}\n` +
        `• Paket: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{nitroInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{nitroInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Username/Email Discord */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Username/Email Discord</label>
            <input
              type="text"
              value={userId}
              onChange={(e) => setUserId(e.target.value)}
              className={inputClass}
              placeholder="Contoh: user#1234 atau email@domain.com"
            />
          </div>

          {/* Step 2: Pilih Paket Nitro */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket Nitro</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {nitroOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
