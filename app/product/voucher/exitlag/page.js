...
// Data produk voucher Exitlag
const exitlagOptions = [
  { label: "1 Month", price: "Rp. 102.000" },
];

// Data info voucher
const exitlagInfo = {
  name: "Exitlag",
  image: "/voucher/Exitlag.jpg",
  desc: "Exitlag Voucher untuk mempercepat koneksi game online dan mengurangi lag. Pilih masa aktif sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Username/email Exitlag
  const [product, setProduct] = useState(exitlagOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Exitlag:\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={exitlagInfo.image}
              alt={exitlagInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{exitlagInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{exitlagInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Username/Email Exitlag */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Username/Email Exitlag</label>
            <input
              type="text"
              value={userId}
