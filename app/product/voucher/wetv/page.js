...
// Data produk voucher WeTV VIP
const wetvProductOptions = [
  { label: "1 Bulan", price: "Rp. 50.000" },
  { label: "3 Bulan", price: "Rp. 130.000" },
  { label: "12 Bulan", price: "Rp. 540.000" },
];

// Data info WeTV
const wetvInfo = {
  name: "WeTV VIP",
  image: "/voucher/WeTV.png",
  desc: "Voucher WeTV VIP untuk akses streaming drama, film, dan variety show Asia favoritmu. Pilih masa aktif sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Email/akun WeTV
  const [product, setProduct] = useState(wetvProductOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order WeTV VIP:\n` +
        `• Email/Akun: ${userId}\n` +
        `• Paket: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{wetvInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{wetvInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Email/Akun WeTV */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Email/Akun WeTV</label>
            <input
              type="text"
              value={userId}
          ...
          {/* Step 2: Pilih Paket VIP */}
          <div className="mb-6">
            <label className={labelClass}>Pilih Paket VIP</label>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {wetvProductOptions.map((opt) => (
                <button
                  key={opt.label}
                  type="button"
