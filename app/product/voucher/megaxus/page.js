...
// Data produk voucher Megaxus (Pulsa)
const pulsaOptions = [
  { label: "10.000", price: "Rp. 13.000" },
  { label: "20.000", price: "Rp. 24.000" },
  { label: "50.000", price: "Rp. 58.000" },
  { label: "100.000", price: "Rp. 115.000" },
  { label: "200.000", price: "Rp. 225.000" },
];

// Data info voucher
const pulsaInfo = {
  name: "Pulsa Megaxus",
  image: "/voucher/megaxus.png",
  desc: "Voucher Pulsa Megaxus untuk berbagai kebutuhan game dan layanan digital. Pilih nominal pulsa sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Username/email Megaxus
  const [product, setProduct] = useState(pulsaOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa Megaxus:\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{pulsaInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{pulsaInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Username/Email Megaxus */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Username/Email Megaxus</label>
            <input
              type="text"
              value={userId}
