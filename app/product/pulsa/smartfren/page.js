...
// Data produk pulsa Smartfren
const smartfrenOptions = [
  { label: "5.000", price: "Rp. 7.000" },
  { label: "10.000", price: "Rp. 12.000" },
  { label: "15.000", price: "Rp. 17.000" },
  { label: "20.000", price: "Rp. 22.000" },
  { label: "50.000", price: "Rp. 52.000" },
  { label: "100.000", price: "Rp. 102.000" },
  { label: "500.000", price: "Rp. 502.000" },
];

// Data info voucher
const smartfrenInfo = {
  name: "Pulsa Smartfren",
  image: "/pulsa/SMARTFREN.png",
  desc: "Top Up Pulsa Smartfren untuk semua kebutuhan komunikasi dan internet. Pilih nominal sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Nomor HP Smartfren
  const [product, setProduct] = useState(smartfrenOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa Smartfren:\n` +
        `• Nomor HP: ${userId}\n` +
        `• Nominal: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={smartfrenInfo.image}
              alt={smartfrenInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{smartfrenInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{smartfrenInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Nomor HP Smartfren */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Nomor HP Smartfren</label>
            <input
              type="text"
              value={userId}
              onChange={(e) => setUserId(e.target.value)}
              className={inputClass}
              placeholder="Contoh: 08881234567"
            />
          </div>
