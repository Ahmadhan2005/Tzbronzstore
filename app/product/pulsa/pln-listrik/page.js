...
// Data produk pulsa PLN Listrik
const plnOptions = [
  { label: "20.000", price: "Rp. 23.000" },
  { label: "50.000", price: "Rp. 53.000" },
  { label: "100.000", price: "Rp. 104.000" },
  { label: "200.000", price: "Rp. 205.000" },
  { label: "500.000", price: "Rp. 510.000" },
  { label: "1.000.000", price: "Rp. 1.020.000" },
];

// Data info voucher
const plnInfo = {
  name: "Pulsa PLN Listrik",
  image: "/pulsa/PLN.png",
  desc: "Voucher Pulsa PLN Listrik untuk isi ulang token listrik prabayar. Pilih nominal sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // ID Pelanggan PLN
  const [product, setProduct] = useState(plnOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa PLN Listrik:\n` +
        `• ID Pelanggan: ${userId}\n` +
        `• Nominal: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{plnInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{plnInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan ID Pelanggan PLN */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan ID Pelanggan PLN</label>
            <input
              type="text"
              value={userId}
