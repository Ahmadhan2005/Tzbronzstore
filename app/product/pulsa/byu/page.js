...
const byuOptions = [
  ...
  { label: "100.000", price: "Rp. 103.500" },
  { label: "150.000", price: "Rp. 153.500" },
  { label: "200.000", price: "Rp. 205.500" },
];

// Data info voucher
const byuInfo = {
  name: "Pulsa by.U",
  image: "/pulsa/BY.u.png",
  desc: "Top Up pulsa by.U untuk semua kebutuhan komunikasi dan internet. Pilih nominal sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Nomor by.U
  const [product, setProduct] = useState(byuOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa by.U:\n` +
        `• Nomor by.U: ${userId}\n` +
        `• Nominal: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{byuInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{byuInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Nomor by.U */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Nomor by.U</label>
            <input
              type="text"
              value={userId}
