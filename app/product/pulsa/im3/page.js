...
// Data produk pulsa Indosat (IM3)
const im3Options = [
  { label: "5.000", price: "Rp. 8.500" },
  { label: "10.000", price: "Rp. 13.500" },
  { label: "20.000", price: "Rp. 23.500" },
  { label: "50.000", price: "Rp. 53.500" },
  { label: "100.000", price: "Rp. 104.000" },
  { label: "200.000", price: "Rp. 204.000" },
  { label: "500.000", price: "Rp. 505.000" },
];
...
const im3Info = {
  ...
  image: "/pulsa/im3.png",
  desc: "Top Up pulsa Indosat (IM3) untuk semua kebutuhan komunikasi dan internet. Pilih nominal sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Nomor HP Indosat
  const [product, setProduct] = useState(im3Options[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa Indosat (IM3):\n` +
        `• Nomor HP: ${userId}\n` +
        `• Nominal: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{im3Info.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{im3Info.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Nomor HP Indosat */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Nomor HP Indosat (IM3)</label>
            <input
              type="text"
              value={userId}
              onChange={(e) => setUserId(e.target.value)}
              className={inputClass}
              placeholder="Contoh: 081512345678"
            />
          </div>
