...
// Data produk pulsa Telkomsel
const telkomselOptions = [
  { label: "5.000", price: "Rp. 7.500" },
  { label: "10.000", price: "Rp. 12.500" },
  { label: "20.000", price: "Rp. 22.500" },
  { label: "25.000", price: "Rp. 27.500" },
  { label: "50.000", price: "Rp. 52.500" },
  { label: "100.000", price: "Rp. 102.500" },
  { label: "200.000", price: "Rp. 203.000" },
...
const telkomselInfo = {
  ...
  image: "/pulsa/telkomsel.png",
  desc: "Top Up pulsa Telkomsel untuk semua kebutuhan komunikasi dan internet. Pilih nominal sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Nomor HP Telkomsel
  const [product, setProduct] = useState(telkomselOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa Telkomsel:\n` +
        `• Nomor HP: ${userId}\n` +
        `• Nominal: ${product}\n` +
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          <div className="w-36 h-36 mb-4 rounded-xl overflow-hidden shadow-lg border-4 border-purple-400 flex items-center justify-center bg-white">
            <img
              src={telkomselInfo.image}
              alt={telkomselInfo.name}
              className="w-full h-full object-contain"
              style={{ maxHeight: "120px", maxWidth: "120px" }}
            />
          </div>
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{telkomselInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{telkomselInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Nomor HP Telkomsel */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Nomor HP Telkomsel</label>
            <input
              type="text"
              value={userId}
