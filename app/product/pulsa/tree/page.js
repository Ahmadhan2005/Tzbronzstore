...
// Data produk pulsa Three (TRI)
const threeOptions = [
  { label: "5.000", price: "Rp. 8.500" },
  { label: "10.000", price: "Rp. 13.000" },
  { label: "20.000", price: "Rp. 23.000" },
  { label: "50.000", price: "Rp. 52.000" },
  { label: "75.000", price: "Rp. 77.000" },
  { label: "100.000", price: "Rp. 102.000" },
  { label: "200.000", price: "Rp. 202.000" },
  { label: "500.000", price: "Rp. 502.000" },
];

// Data info voucher
const threeInfo = {
  name: "Pulsa Three (TRI)",
  image: "/pulsa/TREE.png",
  desc: "Top Up Pulsa Three (TRI) untuk semua kebutuhan komunikasi dan internet. Pilih nominal sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Nomor HP Three
  const [product, setProduct] = useState(threeOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa Three (TRI):\n` +
        `• Nomor HP: ${userId}\n` +
        `• Nominal: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{threeInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{threeInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Nomor HP Three */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Nomor HP Three</label>
            <input
              type="text"
              value={userId}
