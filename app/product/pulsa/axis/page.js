...
const axisInfo = {
  name: "Pulsa Axis",
  image: "/pulsa/axis.png",
  desc: "Top Up pulsa Axis untuk semua kebutuhan komunikasi dan internet. Pilih nominal sesuai kebutuhan, harga murah & proses instan.",
};
...
export default function ProductPage() {
  ...
  const [userId, setUserId] = useState(""); // Nomor HP Axis
  const [product, setProduct] = useState(axisOptions[0].label);
  ...
  const handleCheckout = async () => {
    ...
    if (res.ok) {
      const pesan =
        `Halo Admin, saya ingin order Pulsa Axis:\n` +
        `• Nomor HP: ${userId}\n` +
        `• Nominal: ${product}\n` +
    ...
    <div className="max-w-4xl mx-auto mt-8 p-2 sm:p-4">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className={`md:w-1/3 w-full flex flex-col items-center md:items-start ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 animate-fade-in`}>
          ...
          <h2 className="text-2xl font-extrabold mb-2 text-purple-700 drop-shadow text-center md:text-left">{axisInfo.name}</h2>
          <p className="text-purple-500 text-sm text-center md:text-left">{axisInfo.desc}</p>
        </div>

        {/* Main Form */}
        <div className={`flex-1 ${bgCard} ${borderCard} rounded-xl shadow-lg p-4 sm:p-6 animate-fade-in`}>
          {/* Step 1: Masukkan Nomor HP Axis */}
          <div className="mb-6">
            <label className={labelClass}>Masukkan Nomor HP Axis</label>
            <input
              type="text"
              value={userId}
